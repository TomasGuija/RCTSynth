...
with open('dam/hyperparam.json', 'r') as hfile:
...
def main():
    ...
    N = fh[param['xkey']].shape[-1]
    train_count = max(1, int(round(train_split * N)))
    val_count   = max(1, N - train_count)  # ensure at least one validation case
    # assign indices
    listIDs = [*range(fh[param['xkey']].shape[-1])]
    trainIDs = listIDs[:train_count]
    validIDs = listIDs[train_count:]
